...
private:
	glm::vec2 pos;
public:
	Flow(const glm::vec2& pos);
	~Flow();


	template<typename T> T* Emit(const unsigned int& n,const glm::vec2& vel);
	template<typename T> void EmitIntoSSBO(const unsigned int& n, unsigned int& nParticles, SSBO& ssbo,const glm::vec2& vel);
};
class Emitter3D{
private:
	glm::vec3 pos;
